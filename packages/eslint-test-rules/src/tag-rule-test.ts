...
const imperativeFlatMap = pipe(
  ...
  Effect.flatMap((msg) => {
    // eslint-disable-next-line custom-rules/prefer-match-over-conditionals, no-restricted-syntax -- Testing imperative if in flatMap
    if (msg._tag === 'Command') {
...
const imperativeMap = pipe(
  ...
  Effect.map((e) => {
    // eslint-disable-next-line custom-rules/prefer-match-over-conditionals, no-restricted-syntax -- Testing imperative if in map with _tag
    if (e._tag === 'Right') {
      return e.right;
    }
    return 0;
...
const imperativeReverse = pipe(
  ...
  Effect.succeed<MessageType>({ _tag: 'Command', id: '123' }),
  Effect.flatMap((msg) => {
    // eslint-disable-next-line custom-rules/prefer-match-over-conditionals, no-restricted-syntax -- Testing imperative if with reverse condition
    if ('Command' === msg._tag) {
      return handleMessage(msg);
    }
    return Effect.void;
...
const typeAssertionInFlatMap = pipe(
  ...
  Effect.flatMap((msg) => {
    // eslint-disable-next-line custom-rules/prefer-schema-validation-over-assertions -- Testing type assertion in flatMap
...
const typeAssertionInMap = pipe(
  // eslint-disable-next-line no-restricted-syntax -- Testing first arg in pipe
  Effect.succeed({ type: 'command', id: '123' }),
  // eslint-disable-next-line custom-rules/prefer-schema-validation-over-assertions -- Testing type assertion in map
...
const typeAssertionInTap = pipe(
  ...
  Effect.succeed({ type: 'command', id: '123' }),
  Effect.tap((msg) => {
    // eslint-disable-next-line custom-rules/prefer-schema-validation-over-assertions -- Testing type assertion in tap
    const typed = msg as ProtocolMessage;
...
const doubleAssertion = pipe(
  ...
  Effect.flatMap((data) => {
    // eslint-disable-next-line custom-rules/prefer-schema-validation-over-assertions -- Testing double type assertion
