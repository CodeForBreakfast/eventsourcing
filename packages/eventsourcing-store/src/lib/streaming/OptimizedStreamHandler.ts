 ...
 */
export const makeOptimizedStreamHandler = <TEvent, TStreamId extends string = string>() => pipe(
  // Create references to our handler state
  Effect.all([
    Ref.make(new Map<string, PubSub.PubSub<TEvent>>()),
    Ref.make(0)
  ]),
    ...
    const handler: OptimizedStreamHandlerService<TEvent, TStreamId> = {
      subscribeToStream: (streamId) =>
        Effect.catchAll(
          ...
          (err) => Effect.fail(new StreamingError({
 ...
 */
export const OptimizedStreamHandlerLive = <TEvent = unknown, TStreamId extends string = string>() => {
  const Tag = OptimizedStreamHandler<TEvent, TStreamId>();
  return Layer.effect(
    Tag,
    makeOptimizedStreamHandler<TEvent, TStreamId>()
  );
};