      ...
      Effect.flatMap(([server1, server2, server3]) =>
        pipe(
          ...
          Effect.flatMap(([client1, client2, client3]) => {
            ...
            const msg1 = makeTransportMessage(
              'msg1',
              'test.message',
              JSON.stringify({ server: 1 })
            );
            const msg2 = makeTransportMessage(
              'msg2',
              'test.message',
              JSON.stringify({ server: 2 })
            );
            const msg3 = makeTransportMessage(
              'msg3',
              'test.message',
              JSON.stringify({ server: 3 })
            );
                      ...
                      Effect.flatMap((client1Subscription) =>
                        pipe(
                          ...
                          Effect.map((client1Messages) => {
                            const firstMessage = Array.from(client1Messages)[0];
                            expect(firstMessage?.id).toEqual(expect.stringContaining('broadcast1'));
                          })
                        )
                      )
                    )
